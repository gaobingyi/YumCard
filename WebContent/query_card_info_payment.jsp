...
$(function(){
	...
	$('input:radio[name="parameter_type"]').change(function() {
		...
		if ($selectedvalue == "cardbin") {
			...
			//alert($selectedvalue);
			$('#entry').attr("placeholder", "请输入手机号（账户拥有卡）");
		}
	})
})
</script>
  ...
  <div id="mainbody"><!-- 主体  -->
    <div id="sidebar"><!-- 侧边导航栏  -->
      ...
      <dl>
        ...
        <dd class="current"><a href="query_card_info_payment.jsp">查询卡信息</a></dd>
    ...
    <div id="content"><!-- 内容  -->	
	  <dl>
        <dt>查询卡信息</dt>
        <dd><!-- 存放查询输入框 -->	
		  <form action="/YumCard/QueryCardInfoPaymentServlet" method="post">
...
……"></textarea>
		  ...
		  &nbsp;&nbsp;
		  <input class="checkbox" type="checkbox" name="hasAmount" value="hasAmount" id="3" checked="checked"><label for="3">余额不为0</label>
		  <br>
	  ...
	  <dl>
		...
		if(pager != null) {
			//显示首页
			out.println("<a href=/YumCard/QueryCardInfoPaymentServlet?pageNow=1&queryParameter_new="+queryParameter+"&parameter_type="+parameterType+">首页</a>");
			//显示上一页
			//if (pageNow != 1) {
				out.println("<a href=/YumCard/QueryCardInfoPaymentServlet?pageNow="+((pageNow==1)?pageNow:(pageNow-1))+"&queryParameter_new="+queryParameter+"&parameter_type="+parameterType+">上一页</a>");
			//}       	
			//显示下一页		
			//if (pageNow != pageCount) {
				out.println("<a href=/YumCard/QueryCardInfoPaymentServlet?pageNow="+((pageNow==pageCount)?pageNow:(pageNow+1))+"&queryParameter_new="+queryParameter+"&parameter_type="+parameterType+">下一页</a>");
			//}
			//显示尾页			
			out.println("<a href=/YumCard/QueryCardInfoPaymentServlet?pageNow="+pageCount+"&queryParameter_new="+queryParameter+"&parameter_type="+parameterType+">尾页</a>");
